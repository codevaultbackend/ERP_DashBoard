...
import LiveTransitMap from "../../retail/transit/TransitGoogleMap";
import TransitMapModal from "../../retail/transit/TransitMapModal";

import {
  getHeadTransitById,
  markHeadTransferReceived,
} from "@/features/head-office/transit/api";
import {
  canMarkDelivered,
  formatDate,
  getRouteLabel,
...
export default function HeadTransitDetailContent({
  transferId,
}: {
  transferId?: string;
  basePath?: string;
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const data = await getHeadTransitById(resolvedTransferId);
      setItem(data);
    } catch (err) {
  ...
  const handleMarkDelivered = async () => {
    ...
    try {
      ...
      await markHeadTransferReceived(item.id);
      await loadData();
    } catch (err) {
