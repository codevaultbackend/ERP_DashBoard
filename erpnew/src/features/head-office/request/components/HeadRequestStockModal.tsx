...
export default function HeadRequestStockModal({
  open,
  onClose,
  onSuccess,
}: Props) {
  ...
  useEffect(() => {
    const handleClickOutside = (
      ...
      if (
        ...
        !targetDropdownRef.current.contains(
          target
        )
      ) {
  ...
  useEffect(() => {
    ...
    const fetchTargets = async () => {
      try {
        ...
        const stores = await getDistrictStores();

        const activeStores = stores.filter(
          (store) => store?.is_active !== false
        );

        setOrganizations(activeStores);
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (
                                ...
                                const confirmed =
                                  window.confirm(
                                    "Changing category will clear selected quantities."
                                  );
