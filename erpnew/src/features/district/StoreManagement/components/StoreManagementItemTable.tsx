...
import { AuditIssue, AuditStatus, Category, Item } from "../types";
...
export default function StoreManagementCategoryTable({
  ...
  const selectedCountForCategory = useMemo(() => {
    ...
    const items = itemsMap[auditCategoryId] ?? category?.items ?? [];
    return items.filter((item) => selectedItems[item.id] && !reportedItems[item.id]).length;
  }, [auditCategoryId, safeCategories, itemsMap, selectedItems, reportedItems]);
  ...
  const handleToggleRow = async (category: Category) => {
    ...
    try {
      ...
      setItemsMap((prev) => ({ ...prev, [categoryId]: items }));
  ...
  const getCurrentItems = (category: Category) =>
    itemsMap[category.id] ?? category.items ?? [];
                                      ...
                                      >
                                        ...
                                        <td
                                          className={cn(
                                            ...
                                            isLastItem && isLastRow && "rounded-br-[32px]"
                                          )}
                                        >
                                          {isReported ? (
