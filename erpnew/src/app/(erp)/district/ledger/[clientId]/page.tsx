...
function getLedgerBasePathFromPathname(
...
function getClientIdFromPathname(
  pathname: string | null
) {
...
export default function LedgerClientDetailPage() {
  ...
  const clientId =
    useMemo(() => {
      ...
      return getClientIdFromPathname(
        pathname
      );
  ...
  useEffect(() => {
    ...
    async function loadCustomerLedger() {
      ...
      try {
        ...
        const res =
          await getPaymentsByCustomerByRole(
            clientId
          );
